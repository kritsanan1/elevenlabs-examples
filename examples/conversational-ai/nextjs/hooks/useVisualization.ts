 ...
 */
export function useVisualization(config: VisualizationConfig): VisualizationRenderProps {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      switch (mode) {
        case 'orb':
          drawOrb(ctx, canvas.width / 2, canvas.height / 2, config.isActive, config.isSpeaking);
          break;
        case 'waveform':
          drawWaveform(ctx, canvas.width, canvas.height, config.isActive, config.isSpeaking);
          break;
        case 'spectrum':
          drawSpectrum(ctx, canvas.width, canvas.height, config.isActive, config.isSpeaking);
          break;
      }
  ...
  }, [config.isActive, config.isSpeaking, mode, drawOrb, drawWaveform, drawSpectrum]);
